
## Overview

This project consists of two main stages:

1. **Fine-tuning (Finetune)**  
2. **Feature Extraction and Downstream Task Evaluation**

All configurations are managed through YAML files under the `cfgs/` directory, and datasets are organized in a subject-wise manner.

---

## Pipeline

### Step 1: Fine-tuning

Run the fine-tuning script:

```bash
python finetune.py
````

* This script loads configuration from:

  ```text
  cfgs/arg_ft.yaml
  ```
* The dataset used for fine-tuning is specified in the YAML file via:

  ```yaml
  DATA_ROOT_DIR/DATASET_NAME
  ```

---

### Step 2: Feature Extraction and Downstream Evaluation

Run the feature extraction and downstream task evaluation script:

```bash
python ext_down.py
```

* This script loads configuration from:

  ```text
  cfgs/arg_ex.yaml
  ```
* The dataset path is also specified via:

  ```yaml
  DATA_ROOT_DIR/DATASET_NAME
  ```

---

## Dataset Organization

The dataset should be generated by hdf5_datamaker.py
You should make your own data load-in function. The preprocess and saving part is fixed.

output_file.h5
└── sub{subject_id}                 (Group)
    └── vid{trial_id}               (Group)
        └── sample{slice_id}        (Group: Sequential segments)
            └── eeg                 (Dataset: The actual numerical data)
                ├── Shape: (Channels, Time Points)
                └── Attributes (Metadata):
                    ├── chOrder      : List of channel names, str
                    ├── rsFreq       : Resampling frequency (200Hz)
                    ├── label        : Slice-specific label shape=[n_dim, 1]
                    ├── video_id     : Original trial index
                    └── start_sample : Offset in the original trial

---

